...
void logic_of_game()
{
    ...
    {
        if (dir == DOWNRIGHT)//jezeli direction Ball(a) jest diagonal w prawy d??
        {
            dir = DOWNLEFT;//dir staje diagonal? odrotnej dolnej strony
        }
        else if (dir == UPRIGHT)//tak samo sprawdzenie na kierunek w diagonal gory
        {
            dir = UPLEFT;//zmieniamy kierunek na lewa strone ,ale dalej w g?re
        }
        else//zostaje tylko kierunek  pianowo ,wie? do tej pory  kierunek by? right
        {
            dir = LEFT;//tutaj kierunek zmieniamy na LEFT
        }
    }
    else if (x < 0)//sprawdzanie jezeli x juz jest na linii czy poza granic?
    {
    ...
    if (y >= height) // jezeli wyzej czy rowno dolnej granicy
    {
        ...
        if (dir == DOWNRIGHT)//jezeli kierunek byl w prawy d??
        {
            dir = UPRIGHT;//zmieniamy na praw? g?re
        }
        else if (dir == DOWNLEFT) // je?li kierunek by? w lewy d??
        {
            dir = UPLEFT;//zmieniamy na w g?re na lewo
        }
        else//zostaje nam kierunek poziomowy w d??
        {
            dir = UP;//zmienia si? na g?re
        }
    }
    else if (y < 0)
    {
    ...
    if (platformX >= width - 1)//jezeli X platrofrmy wyjechal lub spotka?  praw? granic?
    {
    ...
    else if (platformX <= 0)//jesli X platfromy wyjecha? lub spotkal lew? granice
    {
    ...
    {
        score_platform += 10;//score dla gracza za platfrome +10 bo to jest ci??ko
        if (dir == DOWNRIGHT)//jesli kierunek  objektu BALL(a)
        {
            dir = UPRIGHT;//zmieniamy
        }
        else if (dir == DOWNLEFT)//jesli kierunek by?
        {
            dir = UPLEFT;//staje tako
        }
        else//zostaje poziom
        {
            dir = UP;//zmieniamy w g?re
        }
    }
}
